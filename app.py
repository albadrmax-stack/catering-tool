...
if submit and (files_input or (selection == "رابط درايف المباشر" and d_url)):
    ...
    if final_files:
        with st.spinner("جاري استخراج البيانات وضبط الأرقام المعكوسة بذكاء..."):
            try:
                ...
                for f_item in final_files:
                    ...
                    # التعديل هنا: تحسين قواعد "العدد" و "الوزن" ليقرأها بذكاء وليس بالترتيب
                    prompt = f"""
                    ...
                    1. 'ملاحظات': يجب دمج محتوى السطر الأصلي للصنف بحيث تكتب (اسم العمود في الفاتورة) ثم نقطتين (:) ثم (القيمة)، وتفصل بين كل عمود وعمود بعلامة ( - ).
                    2. 'اسم الصنف': استخرج اسم المنتج بوضوح. يُمنع منعاً باتاً ترك هذا الحقل فارغاً! يمكنك تجاهل أرقام التعبئة إذا استطعت.
                    3. 'الكمية المشتراة (للتجاهل)': ضع هنا الرقم الموجود في عمود (الكمية) في الفاتورة.
                    4. 'وزن أو سعة الحبة الواحدة': ابحث عن الرقم المرتبط مباشرة بوحدة القياس. (مثال: في عبارة "6*2 كيلو" أو "2*6 كيلو"، الرقم المرتبط بالكيلو هو 2، فاكتب 2. وفي "6*2500 جرام"، اكتب 2500). وإذا كان مفرداً اكتب الوزن.
                    5. 'عدد الحبات داخل الكرتون': هو الرقم الآخر المتبقي في التعبئة. (مثال: إذا كان الوزن 2 في "6*2 كيلو"، فالعدد هو 6. غالباً يكون العدد 4، 6، 12، 24). وإذا كان الصنف مفرداً اكتب 1.
                    6. 'الوحدة الصغيرة': إياك أن تترك هذا الحقل فارغاً! إذا كان الصنف معبأ فاكتب "علبة" أو "جالون". وإذا كان الصنف مفرداً فتكون الوحدة الصغيرة هي نفسها الوحدة الكبيرة.
                ...
                if all_extracted_data:
                    ...
                    st.success("✅ تمت الجراحة رقم 4! الأداة الآن تفرق بين الوزن ومعامل التحويل بذكاء مهما انعكس النص.")
