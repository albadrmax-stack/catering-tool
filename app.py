...
with st.container():
    ...
    # تمت إضافة 'البيان الأصلي للفاتورة' هنا كخيار اختياري
    optional_cols = [
        'رقم الفاتورة أو عرض السعر', 'تاريخ الفاتورة', 'الضريبة', 'الاجمالي مع الضريبة', 'البيان الأصلي للفاتورة'
    ]
...
if submit and (files_input or (selection == "رابط درايف المباشر" and d_url)):
    ...
    if final_files:
        with st.spinner("جاري تحليل البيانات وإجبار الذكاء الاصطناعي على ملء الأعمدة..."):
            try:
                ...
                for f_item in final_files:
                    ...
                    # قالب JSON صارم جداً مضاف إليه "البيان الأصلي للفاتورة"
                    prompt = f"""
                    ...
                    قواعد هامة:
                    1. 'الوحدة الصغيرة': الأصل "علبة" للجامد و"جالون" للسوائل ما لم يذكر غير ذلك.
                    2. 'التصنيف': كلمة واحدة فقط.
                    3. 'الكمية بالوحدة الكبيرة': الرقم الأول في التعبئة. 'الكمية بالوحدة الصغيرة': الرقم الثاني في التعبئة. 'وحدة الوزن الصغيرة': جرام، كيلو، لتر، الخ.
                    4. 'اسم الصنف': بدون أوزان أو أرقام تعبئة.
                    5. 'البيان الأصلي للفاتورة': انسخ الوصف كما هو مكتوب في الفاتورة تماماً بدون أي تعديل.
                    """
                    ...
                    for item in items:
                        ...
                        # حساب معامل التحويل (مستقل لعدم إيقاف باقي العمليات)
                        try:
                ...
                if all_extracted_data:
                    ...
                    # ضمان وجود كل الأعمدة لمنع الأخطاء
                    ...
                    st.success("✅ اكتمل الاستخراج! الترتيب يطابق الإكسل والبيانات ممتلئة بالكامل.")
