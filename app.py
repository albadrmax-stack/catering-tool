...
if submit and (files_input or (selection == "رابط درايف المباشر" and d_url)):
    ...
    if final_files:
        with st.spinner("جاري استخراج البيانات وضبط مسميات الوحدات بدقة..."):
            try:
                ...
                for f_item in final_files:
                    ...
                    # التعديل هنا: تشديد الالتزام بمسمى الوحدة من الفاتورة
                    prompt = f"""
                    أنت محاسب مستودعات دقيق. استخرج البيانات بصيغة JSON فقط.
                    يجب أن يكون الإخراج مطابقاً لهذا الهيكل حرفياً:
                    {{
                        "اسم الشركة / المورد": "",
                        "الرقم الضريبي": "",
                        "رقم السجل التجاري": "",
                    ...
                    1. 'الوحدة الكبيرة' و 'الوحدة الصغيرة': التزم حرفياً بالمسمى المكتوب في الفاتورة (مثال: إذا كتب "تنك" ضع "تنك"، إذا كتب "كيس" ضع "كيس"). لا تستبدلها من عندك بكلمة جالون أو علبة إلا إذا كانت التعبئة صريحة (مثل كرتون داخله حبات).
                    2. 'وزن أو سعة الحبة الواحدة': ابحث عن الرقم المرتبط بالوزن (مثل 2 في 2 كيلو، أو 15 في 15 كيلو طحينة).
                    3. 'عدد الحبات داخل الكرتون': هو الرقم الآخر في التعبئة (مثل 6 في 6*2). وإذا كان مفرد (تنكة، كيس) اكتب 1.
                    4. 'ملاحظات': اكتب (اسم العمود : القيمة) وافصل بـ ( - ).
                    5. 'اسم الصنف': استخرج اسم المنتج بوضوح.
                    6. 'سعر المادة (ر.س)': رقم صافي فقط.
                    """
                    ...
                    for item in items:
                        ...
                        # حماية الوحدة: إذا كانت مفردة (المعامل 1) نضمن تطابق الوحدة الكبيرة والصغيرة حرفياً
                        if qty_large == 1 and l_unit:
                            s_unit = l_unit
                ...
                if all_extracted_data:
                    ...
                    st.success("✅ تمت الجراحة رقم 5! الوحدات الآن مطابقة للفاتورة حرفياً (مثل: تنك).")
